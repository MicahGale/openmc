...
class WeightWindows(IDManagerMixin):
    ...
    def from_xml_element(
        cls, elem: ET.Element, meshes: Dict[int, MeshBase]
    ) -> WeightWindows:
...
def wwinp_to_wws(path: PathLike) -> List[WeightWindows]:
        ...
        if nr == 16:
            ...
            if np.count_nonzero(xyz1) or np.count_nonzero(xyz2):
                raise NotImplementedError(
                    "Custom sphere/cylinder orientations are not supported"
                )
            ...
            ncx, ncy, ncz, nwg = np.fromstring(wwinp.readline(), sep=" ").astype(int)
        else:
    ...
    for grid0, grid_vals, n_pnts in mesh_definition:
        ...
        if (grid_vals[2::3] != 1.0).any():
            raise ValueError("One or more mesh ratio value, qx, " "is not equal to one")
...
class WeightWindowGenerator:
    ...
    def _check_update_parameters(self, params: dict):
        ...
        for key, val in params.items():
            ...
            cv.check_type(
                f'weight window generation param: "{key}"', val, self._MAGIC_PARAMS[key]
            )
