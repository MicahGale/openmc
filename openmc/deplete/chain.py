...
class Chain:
            ...
            if not data.nuclide["stable"] and data.half_life.nominal_value != 0.0:
                ...
                for i, mode in enumerate(data.modes):
                    ...
                    if i == len(data.modes) - 1 and sum_br != 1.0:
                        br = 1.0 - sum(
                            m.branching_ratio.nominal_value for m in data.modes[:-1]
                        )
    ...
    def set_branch_ratios(
            ...
            if len(indexes) == 0:
                if strict:
                    raise AttributeError(
                        "Nuclide {} does not have {} reactions".format(parent, reaction)
                    )
        ...
        if len(missing_products) > 0:
            tail = (
                "{} -> {}".format(k, v) for k, v in sorted(missing_products.items())
            )
            warn(
        ...
        if len(bad_sums) > 0:
            tail = ("{}: {:5.3f}".format(k, s) for k, s in sorted(bad_sums.items()))
            warn(
