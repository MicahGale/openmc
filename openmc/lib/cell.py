...
_dll.openmc_cell_get_rotation.argtypes = [c_int32, POINTER(c_double), POINTER(c_size_t)]
...
_dll.openmc_cell_set_fill.argtypes = [c_int32, c_int, c_int32, POINTER(c_int32)]
...
_dll.openmc_cell_set_rotation.argtypes = [c_int32, POINTER(c_double), c_size_t]
...
_dll.openmc_cell_bounding_box.argtypes = [c_int, POINTER(c_double), POINTER(c_double)]
...
class Cell(_FortranObjectWithID):
    ...
    def fill(self, fill):
        if isinstance(fill, Iterable):
            n = len(fill)
            indices = (c_int32 * n)(*(m._index if m is not None else -1 for m in fill))
    ...
    def rotation(self):
        ...
        _dll.openmc_cell_get_rotation(
            self._index, rotation_data.ctypes.data_as(POINTER(c_double)), rot_size
        )
        ...
        if rot_size == 9:
            ...
            raise ValueError("Invalid size of rotation matrix: {}".format(rot_size))
    ...
    def rotation(self, rotation_data):
        ...
        _dll.openmc_cell_set_rotation(
            self._index,
            flat_rotation.ctypes.data_as(POINTER(c_double)),
