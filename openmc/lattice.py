...
class Lattice(IDManagerMixin, ABC):
    ...
    def clone(self, clone_materials=True, clone_regions=True, memo=None):
        ...
        if self not in memo:
            ...
            if self.outer is not None:
                clone.outer = self.outer.clone(clone_materials, clone_regions, memo)
            ...
            for i in self.indices:
                if isinstance(self, RectLattice):
                    ...
                    if self.ndim == 2:
                        clone.universes[i[0]][i[1]] = self.universes[i[0]][i[1]].clone(
                            clone_materials, clone_regions, memo
                        )
                    else:
                        clone.universes[i[0]][i[1]][i[2]] = self.universes[i[0]][i[1]][
                            i[2]
                        ].clone(clone_materials, clone_regions, memo)
...
class RectLattice(Lattice):
    ...
    def indices(self):
        if self.ndim == 2:
            return list(np.broadcast(*np.ogrid[: self.shape[1], : self.shape[0]]))
        else:
            return list(
                np.broadcast(
                    *np.ogrid[: self.shape[2], : self.shape[1], : self.shape[0]]
                )
            )
    ...
    def ndim(self):
        if self.pitch is not None:
            ...
            raise ValueError(
                "Number of dimensions cannot be determined until "
    ...
    def is_valid_index(self, idx):
        ...
        """
        if self.ndim == 2:
            return 0 <= idx[0] < self.shape[0] and 0 <= idx[1] < self.shape[1]
        else:
            return (
                0 <= idx[0] < self.shape[0]
                and 0 <= idx[1] < self.shape[1]
                and 0 <= idx[2] < self.shape[2]
            )
    ...
    def discretize(
        ...
        if self.ndim != 2:
            raise NotImplementedError(
                "LNS discretization is not implemented " "for 1D and 3D lattices"
            )

        cv.check_value("strategy", strategy, ("degenerate", "lns"))
        cv.check_type(
            "universes_to_ignore", universes_to_ignore, Iterable, openmc.UniverseBase
        )
        cv.check_type(
            "materials_to_clone", materials_to_clone, Iterable, openmc.Material
        )
        cv.check_type(
            "lattice_neighbors", lattice_neighbors, Iterable, openmc.UniverseBase
        )
        ...
        if self.outer is not None and len(lattice_neighbors) == 0:
            ...
            lattice_neighbors = [key(universe) for universe in lattice_neighbors]
        ...
        for j in range(self.shape[1]):
            for i in range(self.shape[0]):
                ...
                if not found:
                    patterns[tuple(map(tuple, pattern))] = {"locations": [(i, j)]}
            ...
            for material in materials_to_clone:
                ...
                for cell in new_universe.get_all_cells().values():
                    ...
                    if cell.fill_type == "material":
                        ...
                        raise (
                            ValueError,
                            "Lattice discretization should not "
    ...
    def from_xml_element(cls, elem, get_universe):
        ...
        uarray = np.array(
            [get_universe(int(i)) for i in get_text(elem, "universes").split()]
        )
        uarray.shape = shape
...
class HexLattice(Lattice):
    ...
    def get_local_coordinates(self, point, idx):
        ...
        if self._num_axial is None:
            ...
            z = point[2] - (
                self.center[2] + (idx[2] + 0.5 - 0.5 * self.num_axial) * self.pitch[1]
            )
        return (x, y, z)
    ...
    def from_xml_element(cls, elem, get_universe):
        ...
        univs = [
            [None for _ in range(max(6 * (n_rings - 1 - r), 1))] for r in range(n_rings)
        ]
        if n_axial > 1:
    ...
    def _repr_axial_slice_x(self, universes):
        ...
        largest_id = max([max([univ._id for univ in ring]) for ring in universes])
        ...
        if orientation == "y":
            ...
            for z in range(n_axial):
                ...
                for r in range(n_rings - 1, 0, -1):
                    ...
                    uarray[-1][-1] = [universes[u_id] for u_id in uarray[-1][-1]]
