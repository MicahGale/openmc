...
class Material(IDManagerMixin):
    ...
    def set_density(self, units: str, density: Optional[float] = None):
        ...
        if units == "sum":
            ...
            cv.check_type(
                'the density for Material ID="{}"'.format(self.id), density, Real
            )
    ...
    def add_element(
        ...
        if len(element) > 2:
            ...
            if element[0].islower():
                msg = (
                    'Element name "{}" should start with an uppercase '
                    "letter".format(element)
                )
                raise ValueError(msg)
            if len(element) == 2 and element[1].isupper():
                msg = 'Element name "{}" should end with a lowercase ' "letter".format(
                    element
                )
                raise ValueError(msg)
            ...
            for token in row:
                if token.isalpha():
                    if token == "n" or token not in openmc.data.ATOMIC_NUMBER:
                        msg = "Formula entry {} not an element symbol.".format(token)
                        raise ValueError(msg)
    ...
    def get_nuclide_atom_densities(
        self, nuclide: Optional[str] = None
    ) -> Dict[str, float]:
    ...
    def get_decay_heat(
        self, units: str = "W", by_nuclide: bool = False, volume: Optional[float] = None
    ) -> Union[Dict[str, float], float]:
    ...
    def get_mass(
        self, nuclide: Optional[str] = None, volume: Optional[float] = None
    ) -> float:
    ...
    def _get_nuclides_xml(
        self,
        nuclides: typing.Iterable[NuclideTuple],
    ...
    def to_xml_element(
        self, nuclides_to_ignore: Optional[typing.Iterable[str]] = None
    ) -> ET.Element:
