...
class CMFDMesh:
    ...
    def grid(self, grid):
        ...
        for i in range(grid_length):
            check_type("CMFD mesh {}-grid".format(dims[i]), grid[i], Iterable, Real)
            check_greater_than(
                "CMFD mesh {}-grid length".format(dims[i]), len(grid[i]), 1
            )
...
class CMFDRun:
    ...
    def _reset_cmfd(self, filename):
        ...
        with h5py.File(filename, "r") as f:
            if "cmfd" not in f:
                raise OpenMCError(
                    "Could not find CMFD parameters in ", "file {}".format(filename)
                )
            else:
