...
class CylinderSector(CompositeSurface):
    ...
    **kwargs : dict
    ...
    def __init__(self, r1, r2, theta1, theta2, center=(0.0, 0.0), axis="z", **kwargs):
        ...
        p2_plane1 = np.array([r1 * cos(phi1), r1 * sin(phi1), 0.0])
        p3_plane1 = np.array([r2 * cos(phi1), r2 * sin(phi1), 0.0])

        p2_plane2 = np.array([r1 * cos(phi2), r1 * sin(phi2), 0.0])
        p3_plane2 = np.array([r2 * cos(phi2), r2 * sin(phi2), 0.0])
...
class Polygon(CompositeSurface):
    ...
    def _constrain_triangulation(self, points, depth=0):
        ...
        if depth > 2:
            raise RuntimeError(
                "Could not create a valid triangulation after 3" " attempts"
            )
