...
class DataLibrary(list):
    ...
    def register_file(self, filename):
        ...
        if path.suffix == ".xml":
            ...
            raise ValueError(
                "File type {} not supported by {}".format(
                    path.name, self.__class__.__name__
                )
            )
    ...
    def export_to_xml(self, path="cross_sections.xml"):
        ...
        common_dir = os.path.dirname(
            os.path.commonprefix([lib["path"] for lib in self])
        )
        if common_dir == "":
            common_dir = "."
    ...
    def from_xml(cls, path=None):
        ...
        if path is None:
            raise ValueError(
                "Either path or openmc.config['cross_sections'] " "must be set"
            )
