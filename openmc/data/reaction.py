...
from .energy_distribution import EnergyDistribution, LevelInelastic, DiscretePhoton
    ...
    for i in range(n_products):
        ...
        if law == 1:
            ...
            if lang == 1:
                ...
                p.distribution = [
                    KalbachMann.from_endf(file_obj, za, zat, projectile_mass)
                ]
    ...
    if (1, 455) in ev.section:
        ...
        if ldg == 0:
            ...
            raise NotImplementedError(
                "Delayed neutron with energy-dependent " "group constants."
            )
...
def _get_photon_products_ace(ace, rx):
    ...
    for i in range(n_photon_reactions):
        ...
        if mftype in (12, 16):
            ...
            energy = (
                ace.xss[
                    ace.jxs[1] + threshold_idx : ace.jxs[1] + threshold_idx + n_energy
                ]
                * EV_PER_MEV
            )
            ...
            raise ValueError("MFTYPE must be 12, 13, 16. Got {0}".format(mftype))
        ...
        if option == 1:
            ...
            for k in range(n_discrete_photon):
                ...
                if law == 1:
                    ...
                    dist.energy = DiscretePhoton(
                        primary_flag, energy, ev.target["mass"]
                    )
