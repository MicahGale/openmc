...
class ResonanceCovariances(Resonances):
    ...
    def ranges(self, ranges):
        ...
        self._ranges = cv.CheckedList(
            ResonanceCovarianceRange, "resonance range", ranges
        )
    ...
    def from_endf(cls, ev, resonances):
        ...
        for _ in range(n_isotope):
            ...
            for j in range(n_ranges):
                ...
                if formalism in [0, 7]:
                    error = (
                        "LRF=" + str(formalism) + " covariance not supported "
                        "for this formalism"
                    )
                ...
                if unresolved_flag in (0, 1):
                    ...
                    erange = _FORMALISMS[formalism].from_endf(
                        ev, file_obj, items, resonance
                    )
...
class ResonanceCovarianceRange:
                ...
                for index2 in indices:
                    for j in range(mpar):
                        if index2 * mpar + j >= index1 * mpar + i:
                            cov_subset_vals.append(
                                cov[index1 * mpar + i, index2 * mpar + j]
                            )
    ...
    def sample(self, n_samples):
        ...
        if formalism == "mlbw" or formalism == "slbw":
            ...
            par_samples = np.random.multivariate_normal(mean, cov, size=n_samples)
            ...
            for sample in par_samples:
                ...
                gx = (
                    sample[4::mpar]
                    if mpar > 4
                    else parameters["competitiveWidth"].values
                )
                gt = gn + gg + gf + gx
        ...
        # Handling RM sampling
            ...
            for sample in par_samples:
                ...
                gg = sample[2::mpar]
                gfa = (
                    sample[3::mpar] if mpar > 3 else parameters["fissionWidthA"].values
                )
                gfb = (
                    sample[4::mpar] if mpar > 3 else parameters["fissionWidthB"].values
                )

                records = []
                for j, E in enumerate(energy):
                    records.append(
                        [energy[j], l_value[j], spin[j], gn[j], gg[j], gfa[j], gfb[j]]
                    )
                columns = [
                    "energy",
                    "L",
...
class MultiLevelBreitWignerCovariance(ResonanceCovarianceRange):
    ...
    def __init__(
        self, energy_min, energy_max, parameters, covariance, mpar, lcomp, file2res
    ):
    ...
    def from_endf(cls, ev, file_obj, items, resonance):
            ...
            for i in range(nls):
                ...
                for j in range(num_res):
                    ...
                    if j < num_res - 1:  # Pad matrix for additional values
                        cov = np.pad(
                            cov, ((0, 4), (0, 4)), "constant", constant_values=0
                        )
        ...
        mlbw = cls(energy_min, energy_max, parameters, cov, mpar, lcomp, resonance)
        return mlbw
...
class SingleLevelBreitWignerCovariance(MultiLevelBreitWignerCovariance):
    ...
    def __init__(
        ...
        super().__init__(
            energy_min, energy_max, parameters, covariance, mpar, lcomp, file2res
        )
        self.formalism = "slbw"
    ...
    def from_endf(cls, ev, file_obj, items, resonance):
        ...
        if lcomp == 1:
            ...
            for i in range(num_short_range):
                ...
                for i, E in enumerate(energy):
                    records.append([energy[i], spin[i], gn[i], gg[i], gfa[i], gfb[i]])
        ...
        rmc = cls(energy_min, energy_max, parameters, cov, mpar, lcomp, resonance)
        return rmc
