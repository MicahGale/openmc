...
def ascii_to_binary(ascii_file, binary_file):
    ...
    with open(str(binary_file), "wb") as binary_file:
        ...
        while idx < len(lines):
            ...
            binary_file.write(
                struct.pack(str("=16i32i{}x".format(record_length - 500)), *(nxs + jxs))
            )
            ...
            binary_file.write(
                struct.pack(str("={}d{}x".format(nxs[0], extra_bytes)), *xss)
            )
...
def get_table(filename, name=None):
    ...
    if name is None:
        ...
        if lib.tables:
            ...
            raise ValueError("Could not find ACE table with name: {}".format(name))
...
class Library(EqualityMixin):
    ...
    def _read_binary(
        ...
        while True:
            ...
            xss = list(
                struct.unpack(str("={}d".format(length)), ace_file.read(length * 8))
            )
    ...
    def from_suffix(cls, suffix):
        ...
        for member in cls:
            if suffix.endswith(member.value):
                return member
        raise ValueError(
            "Suffix '{}' has no corresponding ACE table type.".format(suffix)
        )
