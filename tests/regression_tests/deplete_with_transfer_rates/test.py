...
@pytest.mark.parametrize(
    "rate, dest_mat, power, ref_result",
    [
...
def test_transfer_rates(run_in_tmpdir, model, rate, dest_mat, power, ref_result):
    ...
    integrator = openmc.deplete.PredictorIntegrator(op, [1], power, timestep_units="d")
    integrator.add_transfer_rate(
        "f", transfer_elements, rate, destination_material=dest_mat
    )
