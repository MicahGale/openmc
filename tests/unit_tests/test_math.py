...
def test_t_percentile():
    ...
    test_ts = [
        [openmc.lib.math.t_percentile(p, df) for p in test_ps] for df in test_dfs
    ]
...
def test_calc_pn():
    max_order = 10
    test_xs = np.linspace(-1.0, 1.0, num=5, endpoint=True)

    # Reference solutions from scipy
    ref_vals = np.array(
        [sp.special.eval_legendre(n, test_xs) for n in range(0, max_order + 1)]
    )
...
def test_evaluate_legendre():
    ...
    test_coeffs = [0.5 * (2.0 * l + 1.0) for l in range(max_order + 1)]
    test_xs = np.linspace(-1.0, 1.0, num=5, endpoint=True)
    ...
    test_coeffs = [1.0 for l in range(max_order + 1)]

    test_vals = np.array(
        [openmc.lib.math.evaluate_legendre(test_coeffs, x) for x in test_xs]
    )
...
def test_calc_rn():
    ...
    pol = 0.2  # Latitude
    test_uvw = np.array(
        [np.sin(pol) * np.cos(azi), np.sin(pol) * np.sin(azi), np.cos(pol)]
    )
    ...
    def coeff(n, m):
        return np.sqrt(
            (2.0 * n + 1) * sp.special.factorial(n - m) / (sp.special.factorial(n + m))
        )
    ...
    for n in test_ns:
        for m in range(-n, n + 1):
            if m < 0:
                ylm = pnm_bar(n, np.abs(m), np.cos(pol)) * np.sin(np.abs(m) * azi)
            else:
                ylm = pnm_bar(n, m, np.cos(pol)) * np.cos(m * azi)

            # Un-normalize for comparison
            ylm /= np.sqrt(2.0 * n + 1.0)
            ref_vals.append(ylm)
...
def test_rotate_angle():
    uvw0 = np.array([1.0, 0.0, 0.0])
    phi = 0.0
    mu = 0.0

    # reference: mu of 0 pulls the vector the bottom, so:
    ref_uvw = np.array([0.0, 0.0, -1.0])
...
def test_broaden_wmp_polynomials():
    ...
    test_E = 0.5
    test_dopp = 100.0  # approximately U235 at room temperature
    n = 6

    ref_val = [2.0, 1.41421356, 1.0001, 0.70731891, 0.50030001, 0.353907]
