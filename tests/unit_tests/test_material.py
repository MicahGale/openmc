...
def test_add_components():
    ...
    components = {
        ...
        "O": 1.0,
        "Ag110_m1": 1.0,
        "U": {"percent": 1.0, "enrichment": 4.5},
        "Li": {"percent": 1.0, "enrichment": 60.0, "enrichment_target": "Li7"},
        "H": {
            "percent": 1.0,
            "enrichment": 50.0,
    ...
    with pytest.raises(ValueError):
        m.add_components({"U": {"percent": 1.0, "enrichment": 100.0}})
    with pytest.raises(ValueError):
        m.add_components({"Pu": {"percent": 1.0, "enrichment": 3.0}})
    with pytest.raises(ValueError):
        m.add_components(
            {"U": {"percent": 1.0, "enrichment": 70.0, "enrichment_target": "U235"}}
        )
    with pytest.raises(ValueError):
        m.add_components(
            {"He": {"percent": 1.0, "enrichment": 17.0, "enrichment_target": "He6"}}
        )
    with pytest.raises(ValueError):
...
def test_add_element():
    ...
    m.add_element(
        "H", 1.0, enrichment=50.0, enrichment_target="H2", enrichment_type="wo"
    )
    with pytest.raises(ValueError):
...
def test_add_elements_by_formula():
    ...
    assert elem["Li"] == pytest.approx(4.0 / total_number_of_atoms)
    assert elem["Si"] == pytest.approx(1.0 / total_number_of_atoms)
    ...
    m = openmc.Material()
    m.add_elements_from_formula("Li4SiO4", enrichment=60.0, enrichment_target="Li6")
    ref_dens = {
        "Li6": 0.2666,
        "Li7": 0.1777,
        "Si28": 0.102477,
    ...
    assert elem["Mg"] == pytest.approx(2.0 / total_number_of_atoms)
    assert elem["N"] == pytest.approx(2.0 / total_number_of_atoms)
...
def test_get_activity():
    ...
    assert (
        pytest.approx(m4.get_activity(units="Bq/g", by_nuclide=True)["H3"])
        == 355978108155965.94
    )  # [Bq/g]
    assert (
        pytest.approx(m4.get_activity(units="Bq/cm3")) == 355978108155965.94 * 3 / 2
    )  # [Bq/cc]
    assert (
        pytest.approx(m4.get_activity(units="Bq/cm3", by_nuclide=True)["H3"])
        == 355978108155965.94 * 3 / 2
    )  # [Bq/cc]
    # volume is required to calculate total activity
    m4.volume = 10.0
    assert (
        pytest.approx(m4.get_activity(units="Bq")) == 355978108155965.94 * 3 / 2 * 10
    )  # [Bq]

    # Test with volume specified as argument
    assert (
        pytest.approx(m4.get_activity(units="Bq", volume=1.0))
        == 355978108155965.94 * 3 / 2
    )
...
def test_get_decay_heat():
    ...
    assert (
        pytest.approx(m4.get_decay_heat(units="W/g", by_nuclide=True)["I135"])
        == 40175.15720273193
    )  # [W/g]
    assert (
        pytest.approx(m4.get_decay_heat(units="W/cm3")) == 40175.15720273193 * 3 / 2
    )  # [W/cc]
    assert (
        pytest.approx(m4.get_decay_heat(units="W/cm3", by_nuclide=True)["I135"])
        == 40175.15720273193 * 3 / 2
    )  # [W/cc]
    # volume is required to calculate total decay heat
    m4.volume = 10.0
    assert (
        pytest.approx(m4.get_decay_heat(units="W")) == 40175.15720273193 * 3 / 2 * 10
    )  # [W]

    # Test with volume specified as argument
    assert (
        pytest.approx(m4.get_decay_heat(units="W", volume=1.0))
        == 40175.15720273193 * 3 / 2
    )
...
def test_decay_photon_energy():
    ...
    assert src.integral() == pytest.approx(
        sum(intensity(decay_photon_energy(nuc)) for nuc in m.get_nuclides()), rel=1e-3
    )

    # When the clipping threshold is zero, the intensities should match exactly
    src = m.get_decay_photon_energy(0.0)
    assert src.integral() == pytest.approx(
        sum(intensity(decay_photon_energy(nuc)) for nuc in m.get_nuclides())
    )
