...
rectilinear_mesh.y_grid = np.concatenate(
    (-rectilinear_mesh.y_grid[::-1], rectilinear_mesh.y_grid)
)
rectilinear_mesh.z_grid = np.linspace(-10, 10, 11)

cylinder_mesh = openmc.CylindricalMesh(
    r_grid=np.linspace(0, 10, 23), z_grid=np.linspace(0, 1, 15)
)
...
def test_write_data_to_vtk_size_mismatch(mesh):
    ...
    expected_error_msg = (
        rf"The size of the dataset 'label' \({len(data)}\) should be equal to "
        rf"the number of mesh cells \({mesh.num_mesh_cells}\)"
    )
